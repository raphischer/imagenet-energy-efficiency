...
def evaluate_single(args):
    ...
    args.output_dir = create_output_dir(os.path.join(args.output_dir, 'eval'), args.__dict__)
    try:
        backend = importlib.import_module(f'mlel.ml_{args.backend}.evaluate')
    ...
    for split in ['validation', 'train']:
        ...
        eval_func, model_info = backend.init_evaluation(args, split)
        ...
        # write results
        results = {
            'metrics': backend.finalize_evaluation(eval_result),
            'start': start_time,
            'end': end_time,
            'model': model_info
        }
